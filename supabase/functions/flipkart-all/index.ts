...
serve(async (req) => {
  ...
  try {
    console.log("Fetching all Flipkart offers...");

    if (!FLIPKART_AFFILIATE_ID || !FLIPKART_AFFILIATE_TOKEN) {
      throw new Error("Missing Flipkart affiliate credentials");
    }

    const res = await fetch("https://affiliate-api.flipkart.net/affiliate/offers/v1/all/json", {
      headers: {
...
function transformFlipkartOffers(data: any): any[] {
  try {
    ...
    return data.allOffersList.map((offer: any, index: number) => {
      ...
      return {
        id: `flipkart-${offer.offerId || index}`,
    ...
    console.error("Error transforming Flipkart offers:", error);
    return [];
  }
}
